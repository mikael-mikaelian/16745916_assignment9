...
async function getUser(userID) {
    const user = await fetch(`https://jsonplaceholder.typicode.com/users?id=${userID}`);
...
async function getTodosForUser(userID) {
    const user = await fetch(`https://jsonplaceholder.typicode.com/todos?userId=${userID}`);
    return await user.json();
}